...
def linear_fit_nd(X, y):
    ...
    theta_best = np.linalg.pinv(X_b.T @ X_b) @ X_b.T @ y  # Use pseudo-inverse for stability
    return theta_best
...
def cross_entropy_loss(X, weights, bias, y):
    ...
    probabilities = softmax_safe(scores)
    n_samples = X.shape[1]
    loss = -np.sum(np.log(probabilities[y, np.arange(n_samples)] + 1e-9)) / n_samples  # Fixed: Prevent log(0)
...
def compute_gradients(X, probabilities, y):
    ...
    dL_dW = (1/n_samples) * np.dot((probabilities - y_one_hot), X.T)
    dL_db = (1/n_samples) * np.sum(probabilities - y_one_hot, axis=1, keepdims=True)
    return dL_dW, dL_db
