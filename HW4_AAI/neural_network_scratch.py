...
class TwoLayerNet(object):
  ...
  def __init__(self, input_size, hidden_size, output_size,
    ...
    self.params = {}
    self.params['W1'] = std * torch.randn(hidden_size, input_size,  dtype=dtype, device=device)
    self.params['b1'] = torch.zeros((hidden_size, 1), dtype=dtype, device=device)
    self.params['W2'] = std * torch.randn(output_size, hidden_size, dtype=dtype, device=device)
    self.params['b2'] = torch.zeros((output_size, 1), dtype=dtype, device=device)

      # torch.nn.init.kaiming_uniform_(self.params['W1'])
      # torch.nn.init.kaiming_uniform_(self.params['W2'])
  ...
  def relu(self, X):
    ...
    """
    out = torch.maximum(X, torch.tensor(0.0))  # ReLU: max(0, X)
  ...
  def softmax(self, x):
    ...
    - torch.Tensor: The resulting tensor after applying the softmax function to `x`. Each column of the
      ...
      of the input tensor.
    """
    x_max = torch.max(x, dim=0, keepdim=True).values  # Get max per column (per sample)
    exp_x = torch.exp(x - x_max)  # Subtract max before exponentiation
    softmax_output = exp_x / torch.sum(exp_x, dim=0, keepdim=True)  # Normalize
  ...
  def generate_one_hot(self, shape, labels):
    ...
    one_hot[labels, torch.arange(shape[1])] = 1
  ...
  def cross_entropy_loss(self, probabilities, labels):
    ...
    assert torch.max(labels) < num_classes, "Error: Label index out of range!"

    eps = 1e-10  
    log_probs = torch.log(probabilities + eps)

    correct_log_probs = log_probs[labels, torch.arange(labels.shape[0])]
  ...
  def forward(self, X):
    ...
    self.s1 = torch.matmul(self.params['W1'], X) + self.params['b1']
    self.h1 = self.relu(self.s1)
    self.s2 = torch.matmul(self.params['W2'], self.h1) + self.params['b2']
  ...
  def forward_backward(self, X, labels):
    ...
    loss = self.cross_entropy_loss(self.p, labels)

    batch_size = X.shape[1]

    one_hot_labels = self.generate_one_hot(self.p.shape, labels)
    dL_ds2 = self.p - one_hot_labels

    self.dL_dW2 = (1 / batch_size) * torch.matmul(dL_ds2, self.h1.T)
    self.dL_db2 = (1 / batch_size) * torch.sum(dL_ds2, dim=1, keepdim=True)

    dL_dh1 = torch.matmul(self.params['W2'].T, dL_ds2)
    dL_ds1 = dL_dh1.clone()
    dL_ds1[self.s1 <= 0] = 0

    self.dL_dW1 = (1 / batch_size) * torch.matmul(dL_ds1, X.T)
    self.dL_db1 = (1 / batch_size) * torch.sum(dL_ds1, dim=1, keepdim=True)
    return loss
